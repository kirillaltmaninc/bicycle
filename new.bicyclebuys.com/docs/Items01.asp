...
function getDept
	 dim  rs, sql
	if instr(1,NavID ,"SKU=") > 0  then 	
	
	   vItem=  mid(NavID ,instr(1, NavID ,"SKU=")+4)	   
	   vSection="item"
	end if

	if Request.querystring("SKU")<>"" then 
		vItem=Request.querystring("SKU")
		vSection="item"
	elseif request.querystring("amp;SKU")  <>"" then
		vItem=Request.querystring("amp;SKU")
		vSection="item"
	elseif request.querystring("PID")<>"" then 

	    Set rs = Server.CreateObject("ADODB.Recordset")
		sql="exec spGetSKU " & request.querystring("PID")
        rs.open sql, Conn
		if not rs.eof then
		    vItem= rs.fields("SKU")		
		    if rs.fields("zGood")= 1 then
		        rs.close
		        sql = "exec getitemsku '" & vItem & "'"
		        rs.open sql, Conn
		        vItem= rs.fields("SKU")				        
		    end if
		end if
		vSection="item"
		rs.close
	elseif request.querystring("T")<>"" then
	        Set rs = Server.CreateObject("ADODB.Recordset")

		sql="exec getSubCatByWebType " & request.querystring("T")
        	 rs.open sql, Conn
	
		if not rs.eof then
			vDept = rs.fields("SubCategory")
			if not (isnull(rs.fields("navtype")) or rs.fields("navtype")="") then vSection = rs.fields("navtype")
		else 
			vDept=""
		end if
		rs.close
	end if
	if NavID <>"" then vManufacturer=""
	if NavID = "Pedals" then
		vDept =""
		vSection="pedals"
	end if
end function  
    ...
    call getDept
    if  vSection <> "" and vDept <>"" and vItem <>"" or (vItem<>"" and mTracking="") then       
        checkItemUrls()     
    end if
	...
	if NavID="bikemfg" then
		vSection = "bikes"
	elseif NavID = "Accessory" then 
		vSection = "accessories"
	elseif NavID = "Storage" then
		vSection = "storagesystems"
	elseif NavID = "pedals" then
		vSection = "pedals"
	end if
if    Request.ServerVariables("REMOTE_ADDR")  = "71.190.144.50" then 
   response.write "sku=" &  request.querystring("amp;SKU") &  " navid=" & NavID &  "|section=" & vSection & " item=" & vItem & " dept=" & vDept & " manu=" & vManufacturer & " SKU " & Request.querystring("SKU") & " PID " & request.querystring("PID")
end if	
