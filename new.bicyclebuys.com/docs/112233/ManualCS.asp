...
<%
	'---- encryption key info
	Dim g_KeyLocation, g_Key, g_KeyString, ORIG
...
Sub SendEmail( )
	...
	Conn.Open FileDSN
	sql = "SELECT * FROM Payments WHERE OrderID = " & left(Cart.OrderID,6)  & " order by PaymentID desc  For Browse"           ' & " For Browse For Browse"
	'rs.Open sql, Conn, adOpenStatic,,acCmdText
	rs = conn.execute(sql)
	'response.write(left(Cart.OrderID,6))
	'do until rs.EOF
	'rs.movenext
	'rs.movefirst
	   ' rs.Movelast
	'loop
'response.write(rs.item("PaymentID"))
'response.write(rs.recordcount)

	'pid = rs.fields("PaymentID")
pid = rs.item("PaymentID")
   sql1 = "UPDATE Payments SET ApprovalCode = '" & Cart.CC.AuthCode & "' WHERE PaymentID = " & pid
	Conn.Execute(sql1)
   ...
   if vCC_First = "6" then
      ...
      vCCType = "004"
        ...
        'end if
      	...
      	oRequest( "card_cvNumber") = request.form("cvv2")
