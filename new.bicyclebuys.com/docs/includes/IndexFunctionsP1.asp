...
<%
  dim TestIT,mTracking , xRefer, prodLink, vManufacturerUcase
...
'************************
            ...
            vTMP1 = "newprod" & xx & "nameUrl"
            objTemplate.AddToken vTMP1, 1, lcase(replace(oProd1.pfields.Item("description"), " ","-"))
        ...
        Set rs1 = Server.CreateObject("ADODB.Recordset")
         ...
         while not rs1.eof
            ...
            objTemplate.AddToken vTMP1, 1, oProd1.pfields.Item("description")
				...
				prodLink = replace(oProd1.pfields.Item("description"),"""","")
				prodLink = replace(prodLink,"/"," ")
				prodLink = replace(prodLink,"%20","-")
				prodLink = replace(prodLink,"'","")
				prodLink = replace(prodLink," ","-")
            vTMP1 = "prodLink" & xx & "name"	
            objTemplate.AddToken vTMP1, 1, lcase(prodLink)
...
'end if
            ...
            TestIT = TestIT & "aMostPop[" & counter & "]='" & replace( replace(replace(pfields2.Item("marketingDescription"),chr(13), ""), chr(10),""),"'","\'") & "';" & chr(13)
				...
				descUrl = replace(rs2("description"),"""","")
				descUrl = replace(descUrl,"/"," ")
				descUrl = replace(descUrl,"%20","-")
				descUrl = replace(descUrl," ","-")

				vvMSRP =  msrp(rs2)
				vOUT103 = vOUT103 & "<TD class=""tiny"" align=center valign=top><a href=""/item/" & rs2("SKU") & "/" & lcase(descUrl) &"""><img src=""" & resizepic("/productimages/" & rs2("picture"), rs2("Width_Small"), rs2("Height_Small")) & """ border=""0"" alt=""" & replace(rs2("description"),"""","&quot;")  & """ onmouseover=""setmarkdesc(" & counter & ",this);"" onmouseout=""closeFloat();"" ></a></td>"
				vOUT102 = vOUT102 & "<TD class=""popularfoot"" align=center valign=top>"
				vOUT102 = vOUT102 & "<a href=""/item/" & rs2("SKU") & "/" & lcase(descUrl) &""">" &  replace( rs2("description"), "(","<br>(") & "</a>" & "<BR><span class=""price"">"
				...
				vOUT102 = vOUT102 & "<br><a href=""/item/" & rs2("SKU") & "/" & lcase(descUrl) &""">MORE INFO</a><br>"
...
'************************
			 ...
			 vOUT104 = "" _
              ...
              &   "<INPUT TYPE=""hidden"" NAME=""mTracking"" VALUE=""mostPop4""></FORM>"
				...
				if (counter mod 2 = 0 and counter <> 9 ) then
...
'			end if
            ...
            if vSection = "search" then
    	     ...
    	     elseif ( vSection = "newitems" OR (vSection = "allmfg" AND vDept = "" AND vManufacturer <> "")) then
				...
				.AddToken "showonly", 1, showonlybrands
				.AddToken "pagenav", 1, pagenavout
    	     elseif (vSection = "allmfg" AND vDept <> "" AND vManufacturer <> "") then
                .TemplateFile = TMPLDIR & "productlistfeatMetaLeft.html"
				 vManufacturerUcase = replace(vManufacturer, "_", " ")
				 vManufacturerUcase = UCase(Left(vManufacturerUcase,1)) & Right(vManufacturerUcase,Len(vManufacturerUcase)-1)
				.AddToken "subcategory_name", 1, (vManufacturerUcase & " " & UCase(Left(vDept,1)) & Right(vDept,Len(vDept)-1))
				.AddToken "showonly", 1, showonlybrands
				.AddToken "pagenav", 1, pagenavout
			 else
			 	.TemplateFile = TMPLDIR & "productlistfeatMetaLeft1.html"
				.AddToken "subcategory_name", 1, vUDept
				.AddToken "showonly", 1, showonlybrands
				.AddToken "pagenav", 1, pagenavout
                  .AddToken "blurb", 1, subCatMeta
				.AddToken "leftColumnMob", 3, leftColumnMob
			 end if
...
'zzz Maybe...
   ...
   sub getTop3Related(oProd1)
        dim rs2, descUrlRP, counterRecRelated
		counterRecRelated = 1
        Set rs2 = Server.CreateObject("ADODB.Recordset")
			vOUT105 = ""
			...
			do while not rs2.EOF
			    counterRecRelated = counterRecRelated + 1
				descUrlRP = replace(rs2("description"),"""","")
				descUrlRP = replace(descUrlRP,"/"," ")
				descUrlRP = replace(descUrlRP,"'","")
				descUrlRP = replace(descUrlRP,"%20","-")
				descUrlRP = replace(descUrlRP," ","-")
				tempProd.ClearItem
				tempProd.getfields(rs2)
				vOUT105 = vOUT105 & " <div class=""relatedProductsImg""><a href=""/item/" & rs2("SKU") &"/" & lcase(descUrlRP) & """><img src=""/productimages/" & rs2("picture") & """ width=""75"" border=""0"" alt=""" & rs2("SKU") & "/" & rs2("dbTrack") & """></a></div><div class=""relatedProductsLink""><a class=""relatedProductsLink"" href=""/item/" & rs2("SKU") &"/" & lcase(descUrlRP) & """>" & replace(rs2("description"), "/","") & "</a></div><div class=""relatedProductsPrc"">" &  FormatCurrencyDiscount("<BR>On Special", tempProd.pfields.Item("price"), tempProd.pFields("aDiscount"))  & "</div>"
				'vOUT105 = vOUT105 & " <TR><TD class=""tiny""><img src=""/productimages/" & rs2("picture") & """ width=""35"" border=""0"" alt=""" & rs2("SKU") & "/" & rs2("dbTrack") & """></TD> <TD class=""tiny""><a href=""/item/" & rs2("SKU") & "/" & rs2("description") & """>" & rs2("description") & "</a></TD><TD class=""tiny2"" style=""color: #FB6600"">" &  FormatCurrencyDiscount("<BR>On Special", tempProd.pfields.Item("price"), tempProd.pFields("aDiscount"))  & "</TD></TR>"
			rs2.movenext
			if counterRecRelated > 2 then
				vOUT105 = vOUT105 & "<div style=""clear:both""></div>"
				counterRecRelated = 1
			 end if
   			loop
			rs2.close
    ...
    sub getRecentlyViewed()
        dim rs2, descUrlPV, counterRec, counterRecNum
        dim skus
			...
			skus = left(skus,len(skus)-1)
			    ...
			    counterRecNum = counterRecNum + 1
				if counterRecNum = 2 then
					vOUT100 = vOUT100 & "<div class=""previouslyViewed"">Previously Viewed</div><div style=""clear:both""></div><div class=""previouslyViewedLinks"">"
				 end if
				descUrlPV = replace(rs2("description"),"""","")
				descUrlPV = replace(descUrlPV,"/"," ")
				descUrlPV = replace(descUrlPV,"'","")
				descUrlPV = replace(descUrlPV,"%20","-")
				descUrlPV = replace(descUrlPV," ","-")
					vOUT100 = vOUT100 & "<img src=""images/orange-arrow.gif"" width=""10"" height=""9"" border=0 alt="">""> <a href=""/item/" & rs2("SKU") & "/" & lcase(descUrlPV) & """>" & rs2("description") & "</a>&nbsp;&nbsp;&nbsp;&nbsp;"
					rs2.movenext
				if counterRec > 2 then
					vOUT100 = vOUT100 & "<br/>"
					counterRec = 1
				 end if
                ...
                if (vMSRP / vPrice) > 1.05 Then
                   ...
                   if   (oProd1.pfields.item("RebateCode") = "N" or oProd1.pfields.item("RebateCode")="") then
                       ...
                       sDiscount =oProd1.pfields.Item("aDiscount")
						...
						vSavings = "<div class=""minidescSavings""><span class=""product_save"">You Save:" & FormatCurrency(vMSRP - vPrice + sPrice , 2, 0, 0) & "</span></div><BR>"
                   end if                 '
                end if
            end if
...
'           response.write "<hr>"  & oProd1.pfields.Item("ischildorparentoritem")
    ...
    t = t & "<LINK rel=stylesheet type=""text/css"" href=""menu.css"">" & vbcrlf
        ...
        if isItem <> "-1" then
			...
			't = t & "<link rel=""canonical"" href=""http://www.bicyclebuys.com/"" />" & vbcrlf
        end if




			dim rsSubCatID, vSQLsubcategory, vSQLsubcategoryRes, canDescr, canDescrRepl
           Set rsSubCatID = Server.CreateObject("ADODB.Recordset")
          
 		       
 vSQLsubcategoryRes = "select Subcategory.SubCategory from Subcategory INNER JOIN products on SubCategory.SubCatID = Products.SubCatID and Products.SKU = '"&oProd1.pFields.item("SKU")&"'"
                 rsSubCatID.open vSQLsubcategoryRes, conn
         
                if not rsSubCatID.eof then
                    vSQLsubcategory=rsSubCatID("SubCategory")
                end if
		...
		't = t & "<link rel=""canonical"" href=""http://www.bicyclebuys.com/"& vSection &"/"&vSQLsubcategory&"/"&oProd1.pFields.item("SKU")&""" />" & vbcrlf
			
			canDescr = replace(oProd1.pfields.Item("description"), " ","-")
			canDescrRepl = lcase(replace(canDescr, chr(34),""))
			canDescrRepl = replace(canDescrRepl, "'","")
			canDescrRepl = replace(canDescrRepl, "#","")
			canDescrRepl = replace(canDescrRepl, "/","-")

	
	if vSection="search" then
		t = t & "<link rel=""canonical"" href=""http://www.bicyclebuys.com/"& vSection &""" />" & vbcrlf
	elseif vSQLsubcategory<>"" then
		't = t & "<link rel=""canonical"" href=""http://www.bicyclebuys.com/"& vSection &"/"&vSQLsubcategory&"/"&canDescrRepl&"/"&oProd1.pFields.item("SKU")&""" />" & vbcrlf
		t = t & "<link rel=""canonical"" href=""http://www.bicyclebuys.com/item/"& oProd1.pFields.item("SKU") &"/"&canDescrRepl&""" />" & vbcrlf
	elseif vDept<>"" then
		't = t & "<link rel=""canonical"" href=""http://www.bicyclebuys.com/"& vSection &"/"&vSQLsubcategory&"/"&canDescrRepl&"/"&oProd1.pFields.item("SKU")&""" />" & vbcrlf
		t = t & "<link rel=""canonical"" href=""http://www.bicyclebuys.com/"& vSection &"/"&vDept&"/"" />" & vbcrlf
	end if
...
'        t = t & "<meta name=""keywords"" content=""Discount " &  strDesc  & " " &  strLongDesc &  """>" & vbcrlf
	...
	if mTracking ="goog" or mTracking ="googUK"  or mTracking ="googAU" or   vPriceRange <>"" or vDept="all" or vSorry=1 then
	    t = t & "<META NAME=""ROBOTS"" CONTENT=""FOLLOW,INDEX"">" & vbcrlf
