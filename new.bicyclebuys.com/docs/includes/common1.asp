...
<% 
   ...
   Dim vHeader, vHeaderHTML, vSearchSection, vFooter, vFinalHeader, vFinalFooter, vHeaderNF
   ...
   vHeaderNF = TMPLDIR & "home_base_headerNF.html"
...
'end if
   subCatMeta2=""
            ...
            do while not rs2.eof
               ...
               vOUT1 = vOUT1 & "<span style=""white-space:nowrap""><img src=""images/orange-arrow.gif"" alt="">"" width=""10"" height=""9"" border=0><a href=""/" & vSection & "/" & vTMP2 & "/"">" & rs2("webdisplay") & "</a></span><br/>" & vbcrlf
              rs2.movenext
            loop            
            rs2.close
      ...
      if Not rs1.EOF Then
         ...
         if Not IsEmpty(vTMP1) Then
            Select Case vSect
               ...
               Case Else
					  vSQL2 = "SELECT v.Vendor " _
                        & "FROM dbo.Vendor AS v " _
                        & "WHERE EXISTS ( " _
                        & "SELECT NULL " _
                        & "FROM dbo.products AS p " _
                        & " INNER JOIN dbo.JohnWebNavTypeSubCatIDs AS ns " _
                        & " ON p.SubCatID = ns.SubCatID " _
                        & " INNER JOIN dbo.JohnWebNavType AS nw " _
                        & " ON ns.NavTypeID = nw.NavTypeID " _
                        & " WHERE p.VendID = v.VendID " _
                        & " AND p.Discontinued = 'no' " _
                        & " AND p.webposted = 'yes' " _
                        & " AND CHARINDEX(CAST(p.WebTypeID AS varchar(11)),nw.WebTypes,1) > 0 " _
                        & " );"        & " For Browse"
	    ...
	    vout5="<script language=""javascript"" type=""text/javascript"">function sMan(link) {document.frmM.action=link; document.frmM.submit();}</script><form id=frmM name=frmM METHOD=""post"" action=""""><input type=hidden name=""vDept"" id=""vDept"" value=""" & replace(vDept,"_"," ") & """/></form>"
            ...
            do while not rs2.eof
               ...
               vOUT5 = vOUT5 & "<span style=""white-space:nowrap""><img src=""images/orange-arrow.gif"" alt="">"" width=""10"" height=""9"" border=0><a onclick=""sMan('/manufacturer/" & lcase(vTMP2) & "/" & vSection & "')"" onmouseover=""this.className='mu2'"" onmouseout=""this.className='mu1'"" class=""mu1"">" & rs2("Vendor") & "</a></span><br/>"
               rs2.movenext
            loop
            rs2.close
...
'end if
		...
		vPrefURL = "/item/"
             vOUT1 = vOUT1 & "<tr><td> <FORM METHOD=""post"" action=""https://www.bicyclebuys.com/addtocart/"">" _
...
'   end if
   ...
   With objItemRow
      ...
      .AddToken "itemcustom2", 1, ItemCustom2
       ...
       .AddToken "itemnameURL", 1,  lcase(replace(replace(replace(replace(replace(Trim(ItemName),"%20","-"),"'",""),"/"," "),"""","")," ","-"))
...
Function resizepic(imageurl, w, h)
	if (w = h) then
		imageurl = imageurl & """ width=""120" & """ height=""120"
	elseif (w > h) then
		imageurl = imageurl & """ width=""120"
	else
		imageurl = imageurl & """ height=""120"
	end if
	resizepic = imageurl
End Function
