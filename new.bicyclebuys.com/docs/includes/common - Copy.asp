...
<%    
   ...
   Dim vRemote_IP, vDEBUGGING, vDebug, sqlTrk, vvMSRP, subCatMeta2
   sqlTrk=""
   ...
   class bb_product
    ...
    public sub getDiscountProd(pfields)
        ...
        'If Product webnote = 15, or 7, 18, 2  discontinued don't apply discounts
        if pfields.item("WebNote") = 15   or pfields.item("WebNote") = 18 or pfields.item("WebNote") = 2 or pfields.item("WebNote") = 7 or pFields.item("SKU") = "" then
      ...
      if Not rs1.EOF Then
	    ...
	    vout5="<script language=""javascript"" type=""text/javascript"">function sMan(link) {document.frmM.action=link; document.frmM.submit();}</script><form id=frmM name=frmM METHOD=""post"" action=""""><input type=hidden name=""vDept"" id=""vDept"" value=""" & replace(vDept,"_"," ") & """/></form>"
            ...
            do while not rs2.eof
               ...
               vOUT5 = vOUT5 & "<span style=""white-space:nowrap""><img src=""images/orange-arrow.gif"" alt="">"" width=""10"" height=""9"" border=0><a onclick=""sMan('/manufacturer/" & vTMP2 & "/" & vSection & "')"" onmouseover=""this.className='mu2'"" onmouseout=""this.className='mu1'"" class=""mu1"">" & rs2("Vendor") & "</a></span>"
               rs2.movenext
            loop
            rs2.close
    ...
    set pFields = tempProd.pFields
	    ...
	    'showonlybrands = showonlybrands & "<option value=""" & pFields.item("Vendor") & """>" & pFields.item("Vendor") & "</option>"
         ...
         Else
            vSP = "&nbsp;"
         End If
         if pFields.item("WebNote") = 2  or pFields.item("WebNote") = 15   or pFields.item("WebNote") = 18 or pFields.item("WebNote") = 2 or pFields.item("WebNote") = 7 then
            pFields.item("Discount") = 0
            pFields.item("aDiscount") = 0
            pFields.item("aDiscountType") = 0 
		...
		if ((counter >= (numberperpage * pagenumber) - (numberperpage)) AND (counter < (numberperpage * pagenumber))) then
             ...
             vOUT1 = vOUT1 & "<tr><td> <FORM METHOD=""post"" action=""/addtocart/"">" _
              ...
              & "<INPUT TYPE=""hidden"" NAME=""mSpecialPricing"" VALUE=""" & pFields.Item("mSpecialPricing") & """>" _
               & "     <a href=""" & vPrefURL & pFields.item("SKU") & sqlTrk & """><img src=""" & resizepic("/productimages/" & pFields.item("picture"), pFields.item("Width_Small"), pFields.item("Height_Small")) & """ border=""0"" alt=""" & replace(pFields.item("description"),"""","&quot;") &  """  onmouseover=""setmarkdesc(" & counter + 1 & ",this);"" onmouseout=""closeFloat();""></a></TD>" _
               & "      <TD class=""productlistfoot"" style=""background: transparent;""><a href=""" & vPrefURL & pFields.item("SKU")  &  sqlTrk & """>" & pFields.item("description") & "</a><BR>" _
               & "      " & vSP & "<BR>"
           dim vp2
           if (pFields.item("WebNote") <> 15) then
            ...
            vOUT1 = vOUT1 & "      <a href=""" & vPrefURL & pFields.item("SKU")  & sqlTrk & """>MORE INFO</a>" & vWebNote & "<div align=right>" _
...
'   response.end
   ...
   do while not rs1.EOF
	...
	End If
      ...
      vOUT9 = vOUT9 & vbcrlf & vbcrlf & vbcrlf & "<a href=""/item/" & rs1("SKU") & "/feat""><img src=""" & resizepic("/productimages/" & rs1("picture"), rs1("Width_Small"), rs1("Height_Small")) & """ border=""0"" alt=""" & replace(vTMP4,"""","&quot;") & """ vspace=""10"" ></a><BR>" & vbcrlf _
                    & "<div class=""featuringtext""><a href=""/item/" & rs1("SKU") & "/feat"">" & vTMP4 & "</a></div>" & vbcrlf 
                    vvMSRP =  msrp(rs1) 
                    ...
                    vOUT9 = vOUT9 & "<a href=""/item/" & rs1("SKU") & "/feat"">MORE INFO</a><BR>" & vbcrlf _
...
'************************
      ...
      vTMP4 = Server.HTMLEncode(vTMP4)
      vOUT9 = vOUT9 & vbcrlf & vbcrlf & vbcrlf & "<a href=""/item/" & rs2("SKU") & "/closeOut""><img src=""" & resizepic("/productimages/" & rs2("picture"), rs2("Width_Small"), rs2("Height_Small")) & """ border=""0"" alt=""" & replace(vTMP4,"""","&quot;") & """ vspace=""10"" ></a><BR>" & vbcrlf _
                    & "<div class=""featuringtext""><a href=""/item/" & rs2("SKU") & "/closeOut"">" & vTMP4 & "</a>" & vbcrlf 
            vvMSRP =  msrp(rs2)         
            ...
            elseif Not(tempProd.pfields.item("RebateCode") = "N" or tempProd.pfields.item("RebateCode")="")then
                ...
                vOUT1 = vOUT1  & "<span class=""price"">YOUR SALE PRICE: " & formatcurrencyDiscount("<BR>Promtional Pricing Available!" ,rs2("price"),1) & "</span><BR>"
                    ...
                    vOUT9 = vOUT9 & vWebNote  &  "<a href=""/item/" & rs2("SKU") & "/closeOut"">MORE INFO</a><BR>" & vbcrlf _
                    & "<img name=""feature_divide"" src=""/images/feature_divide.gif"" width=""159"" height=""12"" border=""0"" alt=""""><BR>" & vbcrlf & vbcrlf
      rs2.movenext
   loop
   rs2.close
...
'   end if
   ...
   With objItemRow
      ...
      if ItemAdjust<>0 then
        FreightMsg=FreightMsg & "<hr width=""50%"" size=""1""><div class=""product_freefreight"">(Rebates apply.)</div>"
      end if
		...
		rs1.open vSQL100, conn, 3
		 ...
		 do while not rs1.EOF
			...
			'response.write("objDropdown.options[objDropdown.length] = new Option(""" &  "/manufacturer/" & replace(replace(rs1("Vendor"), " ", "_"),"'", "\'")  & """,""" & rs1("Vendor") & """")
			 response.write("objDropdown.options[objDropdown.length] = new Option(""" &  left(rs1("Vendor"),18) & """,""" &  "/manufacturer/"  & replace(replace(rs1("Vendor"), " ", "_"),"'", "\'")  & """);" & chr(13))
		 	rs1.movenext
		 loop
		 rs1.close
