...
If Request("Save") <> "" and Request("login")="" Then
    ...
    if vBTLN = "" Then vBillingErrString = "LAST NAME;<br/>" &  vBillingErrString
    if vBTFN = "" Then vBillingErrString = "FIRST NAME;<br/>" &  vBillingErrString
    ...
    if (Cart.Info.Custom2="" or IsEmpty(Cart.Info.Custom2) or IsNULL(Cart.Info.Custom2)) Then
        vBillingErrString = vBillingErrString & "COUNTRY;<br/>"
    end if
    if (Cart.Info.ZipPostal) = "" then
        vBillingErrString = vBillingErrString & "PLEASE ENTER 'NONE' IF YOU DON'T HAVE A POSTAL CODE;<br/>"
    end if
end if
...
if Cart.Info.Country = "US" then
    if Cart.Info.StateProvince = "ZZ" then
        vBillingErrString = vBillingErrString & "PLEASE Choose a state;<br/>"
    end if
end if
if Cart.Info.ShipCountry = "US" then
    if Cart.Info.ShipStateProvince = "ZZ" then
        vShippingErrString = vShippingErrString & "PLEASE Choose a shipping state;<br/>"
    end if
end if
...
xwz = InStr(z," ")
xyz = InStr(z,",")
if (x < 1) OR (y < 1) OR (xyz > 0) OR (xwz > 0)  then 
	vBillingErrString = vBillingErrString & "<br/>Your Email is not formatted properly; Enter e-mail address without spaces: (name@company.com)<br/>"
	Cart.Info.Email=""
end if
'if (x < 1) OR (y < 1)  then vBillingErrString = vBillingErrString & "<br/>Your Email is not formatted properly; Enter e-mail address without spaces: (name@company.com)<br/>"
z = request("ShipEmail")
x = InStr(z,"@")
y = InStr(z,".")
xyz = InStr(z,",")
if (x < 1) OR (y < 1) OR (xyz > 0)  then 
	vShippingErrString = vShippingErrString & "<br/>Your Shipping Email is not formatted properly; Enter e-mail address without spaces: (name@company.com)<br/>"
	Cart.Info.Email=""
'if (x < 1) OR (y < 1) then vShippingErrString = vShippingErrString & "<br/>Your Shipping Email is not formatted properly; Enter e-mail address without spaces: (name@company.com)<br/>"
end if
...
if (len(vZipCode) > 5 AND NOT vPlus4) AND vERR = False Then vERR = True
    if vERR Then vBillingErrString = vBillingErrString & "INVALID ZIP CODE;<br/>"
    elseif Cart.Info.Country = "Canada" Then
        vERR = False
        vPlus4 = False
        if (len(Trim(Replace(Cart.Info.ZipPostal, "  ", " "))) < 6) then vBillingErrString = vBillingErrString & "INVALID ZIP CODE;<br/>"
    end if

    '''check phones
    vPhoney = trim(Cart.Info.Phone)
    vPhoneyShip = trim(request("ShipPhone"))
    if len(vPhoney) < 7 or not IsNumeric(vPhoney) then vBillingErrString = vBillingErrString & "INVALID Phone #; Enter Phone # without dashes or spaces: (123456789...)"
    if len(vPhoneyShip) < 7 or not IsNumeric(vPhoneyShip) then vShippingErrString = vShippingErrString & "INVALID Ship Phone #; Enter Phone # without dashes or spaces: (123456789...)"
    ...
    if vBillingErrString <> "" Then vBillingErrString = "<hr><b>The following field(s) are required:</b><br/> " & vBillingErrString

	dim xyzcompany, zcompany, xcompany
	zcompany = trim(request("ShipCompany"))
	xcompany = InStr(zcompany,":")
	if (xcompany > 0)  then 
		vBillingErrString = vBillingErrString & "<br/>Your Company Name should not contain : (colon). Please enter your company name without a colon or leave the field blank<br/>"
		Cart.Info.Company=""
	end if
...
' if browser is in this state then make sure we collect tax
    ...
    ' if there was an error, wrap that error in html
    if vShippingErrString <> "" Then vShippingErrString = "<hr><b>The following field(s) are required:</b><br/>" & vShippingErrString
    ...
    if Cart.Info.ShipCountry = "US" Then
        ...
        ' make sure zip code is just zip code by checking its length and numeric-ness
        if (len(vZipCode) > 5 AND NOT vPlus4) AND vERR = False Then vERR = True
            if vERR Then vShippingErrString = vShippingErrString & "INVALID ZIP CODE;<br/>"
        elseif Cart.Info.ShipCountry = "Canada" Then
            vERR = False
            vPlus4 = False
            if (len(Trim(Replace(Cart.Info.ShipZipPostal, "  ", " "))) < 6) then vShippingErrString = vShippingErrString & "INVALID ZIP CODE;<br/>"
        end if
        dim sql, conn2
...
response.write(Request.Form("Custom6"))
        If Len(Request("Custom6")) >0 Then 
			response.write("yes")
else
			response.write("no")
		end if
        ...
        with objTemplate
        .TemplateFile = TMPLDIR & "checkout.html"
       ...
       adjRebateTotal=0
        ...
        .AddToken "MailSelected", 1, vTMP20
