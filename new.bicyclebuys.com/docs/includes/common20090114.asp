...
<%    
   ...
   class bb_product
    ...
    public sub getDiscountProd(pfields)
        ...
        if pfields.item("WebNote") = 15 or pFields.item("SKU") = "" then
      ...
      Select Case vSect
         Case "S"
            ...
            vSQL = "SELECT top 100 p.*,Vendor.* " _
                 ...
                 & " ORDER BY retailwebprice, p.VendID, description" & " For Browse"
         ...
         vSQLMFG = "SELECT DISTINCT p.vendid, Vendor.* " _
                 ...
                 & " AND webposted LIKE 'yes' " _
                 & " AND (IsChildorParentorItem='1' or IsChildorParentorItem='0' or IsChildorParentorItem='' or IsNull(IsChildorParentorItem,'')='') " _
                 & " ORDER BY p.vendid"  & " For Browse"

         Case "1"    ' Closeout items
      	   vSQL = "SELECT top 100 HTML_Special_SaleItems.*, P.* " _
      	       & "FROM HTML_Special_SaleItems " _
      	       & "INNER JOIN vwWebProducts p ON HTML_Special_SaleItems.Col1_ProductID = p.ProdID " _
...
'     response.write "<HR2>gtsql2:vNavTypes=" & vNavTypes & "  vWebType=|" & vWebType & " | vSection=|" & vSection & " | vManufacturer= |" & vManufacturer & " | vSect= " & vSect 
'end if   
...
'response.write ":" & vWebType & " : " & vSection & " : " & vManufacturer
	...
	elseif (vWebType = "closeouts") then
	   ...
	   vSQLDrop = "SELECT top 100  HTML_Special_SaleItems.*, p.*, Vendor.* " _
            ...
            vSQL ="exec getCloseOutsPage " & apriceRange & ",'" & aVendor & "'" 
	elseif (vWebType = "" AND vSection = "" AND vManufacturer <> "") then
      ...
      vSQLDrop = "SELECT top 100 p.*,Vendor.* " _
           & "FROM vwWebProducts p " _
           & "INNER JOIN Vendor " _
           & "ON Vendor.vendid = p.vendid " _
           & "WHERE 1=1 " _
      ...
      if Not rs1.EOF Then
         ...
         if Not IsEmpty(vTMP1) Then
            ...
            do while not rs2.eof
               ...
               vOUT5 = vOUT5 & "<span style=""white-space:nowrap""><img src=""images/orange-arrow.gif"" alt="">"" width=""10"" height=""9"" border=0><a href=""/manufacturer/" & vTMP2 & "/" & vSection & """>" & rs2("Vendor") & "</a></span>"
	       if (vloop) mod 3 = 0 then 
		   vOUT5 = vOUT5 &  "<br>"
  	       end if
               rs2.movenext
 ...
 End Sub
   ...
   sub getprodlinks3 (vDept, vSection, vManufacturer)
     ...
     ' regular section name here - also handles 'all' dept listing for a section
      ...
      vFilterList = ""
'if    Request.ServerVariables("REMOTE_ADDR")  = "207.237.26.122" then 
' 	response.write("getProdLinks3:" & vSQL)  
'end if
      getprodlist(vSQL)
   ...
   Sub getprodlinks2 (vDept, vSection, vManufacturer, dosearch)
     ...
     ' regular section name here - also handles 'all' dept listing for a section
         ...
         if vSection <> "closeouts" and vSection <> "newitems" then
            ' set the breadcrumb link
'if    Request.ServerVariables("REMOTE_ADDR")  = "207.237.100.71" then 
' 	response.write("getProdLinks2: vNavTypes:" & vNavTypes & " vSection:" & vSection)  
'end if
...
'if    Request.ServerVariables("REMOTE_ADDR")  = "207.237.26.122" then '207.237.26.122
'	response.write vSQL 
'end if
    ...
    set pFields = tempProd.pFields
		...
		if ((counter >= (numberperpage * pagenumber) - (numberperpage)) AND (counter < (numberperpage * pagenumber))) then
         vOUT1 = vOUT1 & "<tr><td> <FORM METHOD=""post"" action=""/addtocart/"">" _
	          ...
	          & "<INPUT TYPE=""hidden"" NAME=""mDiscountAmount"" VALUE=""" & pFields.Item("aDiscount") & """>" _
              & "<INPUT TYPE=""hidden"" NAME=""mSpecialPricing"" VALUE=""" & pFields.Item("mSpecialPricing") & """>" _
               & "     <a href=""" & vPrefURL & pFields.item("SKU") & """><img src=""" & resizepic("/productimages/" & pFields.item("picture"), pFields.item("Width_Small"), pFields.item("Height_Small")) & """ border=""0"" alt=""" & pFields.item("description") &  """></a></TD>" _
               & "      <TD class=""productlistfoot"" style=""background: transparent;""><a href=""" & vPrefURL & pFields.item("SKU") & """>" & pFields.item("description") & "</a><BR>" _
               & "      " & vSP & "<BR>"
dim vp2
              if (pFields.item("WebNote") <> 15) then
		  if pfields.Item("mDiscountAmount") <> 0 then	
   			 	if pfields.Item("mDiscountType") = -1 then 'Dollar
				...
				vp2=vP
                 	...
                 	vOUT1 = vOUT1  & "      <span class=""price"">YOU PAY: " & formatcurrencyDiscount("<BR>On Special<BR>" & formatcurrency(vP2,2,0,0) ,vP,pfields.Item("aDiscount")) & "</span><BR>"
                  else
			...
			vOUT1 = vOUT1 & "<a href=""javascript:void(0)"" onClick=""window.open('/rebate_price.asp?SKU=" & pFields.item("SKU") & "', 'BikePopUp',  'width=520,height=400,toolbar=0,scrollbars=1,screenX=50,screenY=50,left=50,top=50')""><font color=blue>Click here to View the Instant Rebate<br>price you will see in the Checkout</font></a><br>"
              ...
              vOUT1 = vOUT1 & "      <a href=""" & vPrefURL & pFields.item("SKU") &""">MORE INFO</a>" & vWebNote & "<div align=right>" _
			  & vItemOptions & "<input name=""SUBMIT"" VALUE=""ADD"" type=image src=""images/addtocart.jpg"" alt=""View Cart"" style=""margin: 5px 0 0 0;""></div>" _
	  ...
	  do while (counter > 0)
	  	if (pagenumber = loccounter) then
			...
			pagenavout = pagenavout & "<a href=""/?pg=pg&c=" & vSection & "&d=" & replace(vDept, "'", "\'") & "&m=" & vManufacturer & "&price=" & vpriceRange & "&numberperpage=" & numberperpage & "&pagenumber=" & loccounter & """>" & loccounter & "</a> "
		end if
            ...
            vOUT1 = vOUT1 & "   <TR>" _
                ...
                if (rs100("WebNote") <> 15) then
                    if pfields.Item("aDiscount") <> 0 then	
                 	...
                 	vOUT1 = vOUT1  & "      <span class=""price"">YOU PAY:. " & formatcurrencyDiscount("<BR>On Special<BR>" & formatcurrency(vP2,2,0,0) ,vP,tempProd.pfields.item("aDiscount")) & "</span><BR>"
                else
                    vOUT1 = vOUT1  & "      <span class=""price"">YOU PAY:. " & formatcurrency(vP,2,0,0) & "</span><BR>"
	 ...
	 'exit function
      ...
      vOUT1 = ""
		...
		vSQL = "SELECT top 100 [Products Children].*,p.*,Size.* " _
		     ...
		     & "INNER JOIN (Products p INNER JOIN Size ON p.SizeID = Size.SizeID) " _
		     & "ON [Products Children].ChildProdID = p.ProdID " _
		     & "WHERE [Products Children].ProdID=" & ProdID _
		     & " AND ((p.Discontinued LIKE 'yes' AND p.QtyAvailable > 0) or (p.Discontinued LIKE 'no')) " _
...
'		response.write "<pre>" & vSQL & "</pre><br>"
		...
		If rschild.EOF then
			vSQL = "SELECT top 100 [Products Children].*,p.* " _
			     & "FROM [Products Children] " _
			     & "INNER JOIN vwWebProducts p ON [Products Children].ChildProdID = p.ProdID " _
			     & "WHERE [Products Children].ProdID=" & ProdID & " AND ((p.Discontinued LIKE 'yes' AND p.QtyAvailable > 0) or (p.Discontinued LIKE 'no'))" _
			     '& " For Browse"
			rschild.close
'			response.write "<pre>" & vsql & "</pre><br>"
...
'					vUseDesc = vUseDesc & " Q:" & rsShow("QtyAvailable") & " D:" & rsShow("Discontinued")
					...
					vOUT105 = vOUT105 & chr(9) & chr(9) & "<option value=""" & rsShow("SKU") & """>" & vUseDesc & chr(13)
					rsShow.movenext
				wend
				vValue = left(vValue, Len(vValue) - 1)
				vValue1 = left(vValue1, Len(vValue1) - 1)
   ...
   do while not rs1.EOF
	...
	End If
      ...
      vOUT9 = vOUT9 & vbcrlf & vbcrlf & vbcrlf & "<a href=""/item/" & rs1("SKU") & """><img src=""" & resizepic("/productimages/" & rs1("picture"), rs1("Width_Small"), rs1("Height_Small")) & """ border=""0"" alt=""" & vTMP4 & """ vspace=""10"" ></a><BR>" & vbcrlf _
                    ...
                    & "<span class=""price"">You Pay: " & FormatCurrencyDiscount("<BR>On Special", rs1("price"), tempProd.pfields.item("aDiscount")) & "</span><br>" & vWebNote  & vbcrlf _
...
function getcatheader(vMetaTitle, vMetadescription, vMetaKeywords)
   ...
   vMTitle = vMetaTitle & ""
   if vMTitle = "" Then vMTitle = "BicycleBuys.com | Online Bike Shop | Bicycles | Bike Parts | Frames | Discounted"
...
'************************
      ...
      vOUT9 = vOUT9 & vbcrlf & vbcrlf & vbcrlf & "<a href=""/item/" & rs2("SKU") & """><img src=""" & resizepic("/productimages/" & rs2("picture"), rs2("Width_Small"), rs2("Height_Small")) & """ border=""0"" alt=""" & vTMP4 & """ vspace=""10"" ></a><BR>" & vbcrlf _
                    ...
                    & "<span class=""price"">You Pay: " & FormatCurrencyDiscount("",rs2("price"),tempprod.pfields("aDiscount")) & "</span><br>" & vbcrlf _
                    & vWebNote  &  "<a href=""/item/" & rs2("SKU") & """>MORE INFO</a><BR>" & vbcrlf _
                    & "<img name=""feature_divide"" src=""/images/feature_divide.gif"" width=""159"" height=""12"" border=""0"" alt=""""><BR>" & vbcrlf & vbcrlf
      rs2.movenext
   loop
   rs2.close
	...
	For I = LBound(poparray) To UBound(poparray)
	 ...
	 ' response.write "<hR>" & vSQL
	   ...
	   if NOT rs1.EOF then
		  ...
		  vTMP1 = vTMP1 & "<TD class=""driveitems"" align=center><a href=""" & lcase(linkarray(I)) & """><img src=""" & resizepic("/productimages/" & rs1("picture"), rs1("Width_Small"), rs1("Height_Small")) & """  border=0 alt =""" &  textarray(I)  & """></a></TD>" & chr(13)
		  vTMP2 = vTMP2 & "<TD class=""driveitems"" align=center><a href=""" & lcase(linkarray(I)) & """>" & textarray(I) & "</a></TD>" & chr(13)
		...
		rs1.open vSQL100, conn, 3
		 ...
		 do while not rs1.EOF
			...
			'response.write("objDropdown.options[objDropdown.length] = new Option(""" &  "/manufacturer/" & replace(replace(rs1("Vendor"), " ", "_"),"'", "\'")  & """,""" & rs1("Vendor") & """")
			 response.write("objDropdown.options[objDropdown.length] = new Option(""" &  rs1("Vendor") & """,""" &  "/manufacturer/"  & replace(replace(rs1("Vendor"), " ", "_"),"'", "\'")  & """);" & chr(13))
		 	rs1.movenext
		 loop
		 rs1.close
