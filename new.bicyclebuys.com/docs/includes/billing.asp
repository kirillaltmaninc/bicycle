...
conn.close
   ...
   elseif Cart.Info.PaymentType = "PayPal" Then
      epayment = epayment & "PayPal" & vbcrlf
      epayment = epayment & "  Card Holder: " & Cart.Info.CCName & vbcrlf
	  epayment = epayment & "  Email: " & Cart.Info.Email & vbcrlf
   Else
   ...
   Elseif Cart.Info.PaymentType = "PayPal" Then
      epaymentI = epaymentI & "Payment: PayPal" & vbcrlf
   Else
   ...
   'ettl = ettl & "Subtotal:                    " & formatcurrency(Cart.Gridtotal,2,0,0) & vbcrlf
        ...
        if (TotalDiscount15 > 0) then
            ettl = ettl & "Subtotal:                    " & formatcurrency(Cart.GridTotal - TotalDiscount15,2,0,0) & vbcrlf
        elseif (adjRebateTotal < 0) then
            ettl = ettl & "Rebate Total:                    " & formatcurrency(adjRebateTotal,2,0,0) & vbcrlf
            ettl = ettl & "Subtotal:                    " & formatcurrency((Cart.GridTotal-vAdjustTotal) + adjRebateTotal,2,0,0) & vbcrlf
   ...
   'ettl = ettl & "Total:                       " & formatcurrency(Cart.Total,2,0,0) & vbcrlf & vbcrlf
            if (TotalDiscount15 > 0) then
                ettl = ettl & "Total:                       " & formatcurrency(Cart.Total - TotalDiscount15,2,0,0) & vbcrlf
            elseif (adjRebateTotal < 0) then
                 ettl = ettl & "Total:                       " & formatcurrency((Cart.Total-vAdjustTotal) + adjRebateTotal,2,0,0) & vbcrlf
            else
                ettl = ettl & "Total:                       " & formatcurrency(Cart.Total,2,0,0) & vbcrlf
            end if
...
'DON CODE FOR CYBER SCREWUP XXXXX
		...
		'----------------- CREDIT CARD PROCESSING -----------------------------------------------------
			 ...
			 ''''''''''''''''''''''''''''''''''''''''''''''''''''''''
			   ...
			   rs100.open sql,Conn,3
				...
				dim fName, lName
				fName = oRequest( "billTo_firstName" )
				lName = oRequest( "billTo_lastName" )
		...
		'----------------- PayPal --------------------------------------------------------------
				...
				If Cart.Info.PaymentType = "PayPal" Then
...
my_paypal_email = "paypal.sales@bicyclebuys.com"                                                           
...
'Response.Redirect "https://www.paypal.com/cgi-bin/webscr"
	...
	if (Cart.Info.ShipStateProvince <> "NY") then
      ...
      vPaymentTypes = vPaymentTypes & "<option VALUE=""PayPal"""
      If Cart.Info.PaymentType = "PayPal" Then vPaymentTypes = vPaymentTypes & "SELECTED"
      vPaymentTypes = vPaymentTypes & ">PayPal</option>"
