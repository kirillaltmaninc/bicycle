       ...
       adjRebateTotal=0
	    ...
	    .AddToken "cartid", 1, vCartID
         ...
         '.AddToken "grandtotal", 1, vGrandTotal  
          if (TotalDiscount15 > 0) then
                .AddToken "grandtotal", 1, FormatCurrency(vGrandTotal - TotalDiscount15,2,0,0)
                'response.Write("TotalDiscount15  - " & TotalDiscount15)
            elseif (adjRebateTotal < 0) then
                'response.Write("adjRebate  - " & adjRebate)
                .AddToken "grandtotal", 1, FormatCurrency((vGrandTotal-vAdjustTotal) + adjRebateTotal,2,0,0)
            else
                .AddToken "grandtotal", 1, vGrandTotal 
            end if
