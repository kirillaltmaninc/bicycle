...
dim pwdResetMsg, adjRebate, adjRebateTotal
...
if (len(vZipCode) > 5 AND NOT vPlus4) AND vERR = False Then vERR = True
    ...
    if len(vPhoney) < 7 then vBillingErrString = vBillingErrString & "INVALID Phone #;"
    if len(vPhoneyShip) < 7 then vShippingErrString = vShippingErrString & "INVALID Ship Phone #;"
...
' if browser is in this state then make sure we collect tax
	...
	Cart.Info.OrderCustom6 = left(replace(replace(replace(Session("ReferredBy"),"'",""),"http://www.bicyclebuys.com/","B/"),"http://www.google.com/","G/"),50)
      ...
      For each item in Cart.Items 
       ...
       adjRebate=vAdjustTotal*Item.Quantity
       .AddToken "adjusttotal", 1, rebateHtml(adjRebate) 
       if (vAdjustTotal < 0) then
      ...
      Next     
        ...
        elseif (adjRebate < 0) then
            
            if (vAdjustTotal < adjRebate) then
                .AddToken "cgridtotal", 1, FormatCurrency((Cart.GridTotal-vAdjustTotal) + adjRebateTotal,2,0,0)
            else
                .AddToken "cgridtotal", 1, FormatCurrency(Cart.GridTotal+vAdjustTotal)
       ...
       ' if (adjRebateTotal < vAdjustTotal) then
        'else
